...
class IdConfirmation extends StatefulWidget {
  final bool showSkip;
  const IdConfirmation({super.key, this.showSkip = true});
...
class _IdConfirmationState extends State<IdConfirmation> {
  ...
  // Update _saveInfo method:
  Future<void> _saveInfo() async {
    if (!mounted || context.read<IdConfirmationProvider>().isLoading) return;
    ...
    final provider =
        Provider.of<IdConfirmationProvider>(context, listen: false);
    final success = await provider.uploadIdCards(
      _pickedFrontSideImageFile!.path,
      _pickedBackSideImageFile!.path,
    );
    ...
    _showMessage(
      success
          ? 'idConfirmation.success.saved'
          : provider.error ?? 'idConfirmation.errors.saveFailed',
      !success,
    );
        ...
        children: [
          ...
          Consumer<DriverRegistrationProvider>(
            builder: (context, provider, child) {
              ...
              final imageToShow =
                  imageFile ?? (savedImage != null ? File(savedImage) : null);
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: screenHeight * 0.001),
                CustomButton(
                  text: context.watch<IdConfirmationProvider>().isLoading
                      ? 'Loading...'
                      : 'idConfirmation.save'.tr(),
                  textStyle: AppTextTheme.getLightTextTheme(context).bodyLarge!,
                  onPressed: () => _saveInfo(), // Wrapped in closure
                  borderRadius: 44,
                ),
              ],
            ),
