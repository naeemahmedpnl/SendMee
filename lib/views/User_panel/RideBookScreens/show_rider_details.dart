...
//     // Extract passenger ID from the correct structure in tripDetails
//     final passengerId = widget.tripDetails['passenger']?['_id'] ?? 
//                        widget.tripDetails['tripDetails']?['passenger']?['_id'];

//     if (passengerId != null) {
//       log('Listening for notifications for passenger ID: $passengerId');
...
//       });
//     } else {
//       log('WARNING: Could not find passenger ID in tripDetails');
//     }

//     // Extract trip ID from the correct structure
//     final tripId = widget.tripDetails['_id'] ?? widget.tripDetails['tripDetails']?['_id'];
...
//       // Extract IDs from the correct structure
//       final tripId = widget.tripDetails['_id'] ?? 
//                      widget.tripDetails['tripDetails']?['_id'];
                     
//       final driverId = widget.initialTripDetails['driver']?['_id'] ?? 
//                        widget.tripDetails['tripDetails']?['driver']?['_id'];
...
//       });
//     } else {
//       log('ERROR: Could not confirm payment - trip ID is null');
//     }
//   }
...
//     // Handle either structure of tripDetails
//     final pickup = _parseLatLng(widget.initialTripDetails['pickup'] ?? 
//                   widget.tripDetails['tripDetails']?['pickup'] ?? "0,0");
                  
//     final destination = _parseLatLng(widget.initialTripDetails['destination'] ?? 
//                        widget.tripDetails['tripDetails']?['destination'] ?? "0,0");
    
//     // Get driver location from the appropriate structure
//     LatLng driverLocation;
//     if (widget.initialTripDetails['driverLocation'] != null) {
//       driverLocation = LatLng(
//           widget.initialTripDetails['driverLocation']['latitude'],
//           widget.initialTripDetails['driverLocation']['longitude']);
//     } else {
//       // Fallback to pickup location if driver location is missing
//       driverLocation = pickup;
//       log('WARNING: Driver location not found, using pickup location instead');
//     }
...
//     // Safely extract IDs with fallbacks at different levels of nesting
//     final passengerId = widget.tripDetails['passenger']?['_id'] ?? 
//                        widget.tripDetails['tripDetails']?['passenger']?['_id'] ?? 'null';
    
//     // Driver data extraction based on the structure in your logs
//     final driverDetails = widget.initialTripDetails['driver'] ?? 
//                          widget.tripDetails['tripDetails']?['driver'];
    
//     final driverId = driverDetails?['userid'] ?? driverDetails?['_id'] ?? 'null';
//     final driverPhone = driverDetails?['phone'] ?? 'null';
//     final driverName = driverDetails?['username'] ?? "Unknown";
//     final driverRating = driverDetails?['ratingAverage'] ?? 0.0;
//     final driverTripsCount = driverDetails?['tripsCount'] ?? 0;
//     final driverProfilePic = driverDetails?['profilePicture'] ?? "";
...
//                 widget.tripDetails['tripDetails']?['driverEstimatedFare'] ?? 0.0;
...
//                               driverRating: "${driverRating.toStringAsFixed(1)} ($driverTripsCount Trips)",
//                               driverImageUrl: driverProfilePic,
//                               userId: passengerId,
//                               driverId: driverId,
//                               phoneNumber: driverPhone,
//                             ),
...
import 'package:google_maps_flutter/google_maps_flutter.dart';
import 'package:provider/provider.dart';
import 'package:sendme/services/fcm_notifcation.dart';
import 'package:sendme/utils/constant/api_base_url.dart';
...
class _ShowRiderDetailsState extends State<ShowRiderDetails> {
  ...
  void initState() {
    ...
    // Check if we need to show a delivery proof dialog
    _checkForPendingDeliveryProof();
  }

  Future<void> _checkForPendingDeliveryProof() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final shouldShowDialog =
          prefs.getBool('show_delivery_proof_dialog') ?? false;

      if (shouldShowDialog) {
        final imageUrl = prefs.getString('latest_delivery_proof');

        if (imageUrl != null && mounted) {
          // Remove both the flag and the image URL from SharedPreferences
          await prefs.remove('show_delivery_proof_dialog');
          await prefs.remove('latest_delivery_proof');

          log('Removed pending delivery proof from SharedPreferences');
        }
      }
    } catch (e) {
      log('Error checking for pending delivery proof: $e');
    }
  }

  Future<void> _storeEstimatedFare() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final fareAmount = widget.initialTripDetails['driverEstimatedFare']
              ?.toStringAsFixed(2) ??
          '0.00';
      await prefs.setString('estimated_fare', fareAmount);
      log('Saved estimated fare to prefs: $fareAmount');
    } catch (e) {
      log('Error saving estimated fare to prefs: $e');
    }
  }

  void _navigateToPaymentScreen(String? imageUrl) {
    try {
      ...
      final isParcelDelivery = serviceType == 'parcel';
      final fareAmount = widget.initialTripDetails['driverEstimatedFare']
              ?.toStringAsFixed(2) ??
          '0.00';

      // Always store fare for redundancy
      _storeEstimatedFare();
      ...
      Navigator.pushReplacement(
        ...
        MaterialPageRoute(
          builder: (context) => PaymentMethodScreen(
            ...
            deliveryProofImage: normalizedUrl,
            isParcelDelivery: isParcelDelivery,
          ),
        ),
      );
    } catch (e) {
      log('Error navigating to payment screen: $e');

      // Fallback navigation with minimal arguments
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
  ...
  socket?.on(widget.tripDetails['passenger']['_id'],
      (notification) async {
    log('Received notification: $notification');
    ...
    if (startKeywords.any((keyword) => messageLower.contains(keyword))) {
      title = tr('ride_tracker.trip_started');
    } else if (completedKeywords.any((keyword) => messageLower.contains(keyword))) {
      title = tr('ride_tracker.trip_completed');
    } else {
      title = tr('ride_tracker.notification');
    }
    ...
    bool isCompletionMessage = completionKeywords.any(
    ...
    if (isCompletionMessage) {
      ...
      if (mounted) {
        ...
        try {
          final fare = widget.initialTripDetails['driverEstimatedFare']?.toStringAsFixed(2) ?? '0.00';
  ...
  Widget build(BuildContext context) {
    ...
    // Declare variables outside try-catch with default values
    String passengerId = 'null';
    String driverId = 'null';
    String driverPhone = 'null';

    // Assign within try-catch
    try {
      // Get passenger ID
      passengerId = widget.tripDetails['tripDetails']?['passenger']?['_id'] ?? 'null';
      
      // Get driver ID - just use the userid string directly
      driverId = widget.initialTripDetails['driver']?['userid']?.toString() ?? 'null';
      
      // For phone, you'd need to get this from another source since it's not in the structure
      // Or use the driver ID to fetch the driver's phone if needed
      driverPhone = widget.tripDetails['tripDetails']?['passenger']?['phone'] ?? 'null';
                                ...
                                GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    child: Center(
                                      child: Text(
                                        'ride_tracker.cancel_ride'.tr(),
                                        style: AppTextTheme.getDarkTextTheme(
                                                context)
                                            .titleLarge,
                                      ),
                                    ),
